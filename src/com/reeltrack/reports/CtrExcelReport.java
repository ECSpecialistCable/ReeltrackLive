...
public class CtrExcelReport {
    ...
    public CtrExcelReport(PageContext pageContext, DbResources resources) {
	...
	public HSSFWorkbook writeUserExcel(String jobCode, String basePath) throws Exception {
		...
		HSSFSheet sheet = wb.createSheet("CTR Report");
		HSSFRow row;
		HSSFCell cell;
		...
		for (int i = 0; i < reels.howMany(); i++) {
            ...
            row.createCell((short)nextNum++).setCellValue(current.getCTRNumber());
            row.createCell((short)nextNum++).setCellValue(current.getCTRDateString());
            row.createCell((short)nextNum++).setCellValue(current.getCTRSentString());
            if(current.getCTRFile().equals("")) {
                row.createCell((short) nextNum++).setCellValue("No");
            } else {
                row.createCell((short) nextNum++).setCellValue("Yes");
            }
