...
if(request.getParameter("pageNum") != null) {
    ...
    session.setAttribute("received/scrapped.jsp", pageNum);
} else {
    if(session.getAttribute("received/scrapped.jsp") != null) {
        pageNum = (Integer)session.getAttribute("received/scrapped.jsp");
    }
}
...
if(session.getAttribute("received_scrapped")!=null) {
    content = (Reel)session.getAttribute("received_scrapped");
}
...
session.setAttribute("received_scrapped",content);
...
<% dbResources.close(); %>
<html:begin />
<admin:title heading="Reels" text="Scrapped" />

<admin:subtitle text="Search" />
    <admin:box_begin text="Search Reels"   name="Search" open="false"/>
    <form:begin_selfsubmit name="search" action="received/scrapped.jsp" />
        <%
        tempURL = "";
...
<% if(dosearch) { %>
    ...
    <admin:box_begin text="Reels" name="results" url="received/scrapped.jsp" pages="<%= Integer.toString(pages) %>" pageNum="<%= Integer.toString(pageNum) %>" />


    <listing:begin />
        <listing:header_begin />
            <listing:header_cell width="55" first="true" name="CRID #" column="<%= Reel.CR_ID_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/scrapped.jsp" />
            <listing:header_cell name="Reel Tag" column="<%= Reel.REEL_TAG_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/scrapped.jsp" />
            <listing:header_cell name="Cable Description" column="<%= Reel.CABLE_DESCRIPTION_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/scrapped.jsp" />
            <listing:header_cell width="75" name="Status" column="<%= Reel.STATUS_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/scrapped.jsp" />
...
<admin:set_tabset url="received/_tabset_default.jsp" thispage="scrapped.jsp" />
<html:end />
