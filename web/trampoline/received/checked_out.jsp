...
if(session.getAttribute("received_checked_out")!=null) {
    content = (Reel)session.getAttribute("received_checked_out");
}
...
session.setAttribute("received_checked_out",content);
...
<% dbResources.close(); %>
<html:begin />
<admin:title text="Reels Checked Out" />

<admin:subtitle text="Search" />
    <admin:box_begin />
    <form:begin_selfsubmit name="search" action="received/checked_out.jsp" />
        <%
        tempURL = "";
...
<% if(dosearch) { %>
    <% if(contents.howMany() > 0) { %>
        <admin:search_listing_pagination text="Reels Found" url="received/checked_out.jsp" 
    ...
    <listing:begin />
        <listing:header_begin />
            <listing:header_cell width="55" first="true" name="CRID #" column="<%= Reel.CR_ID_COLUMN %>" ascending="<%= "" + ascending %>" match="<%= column %>" url="<%= "received/checked_out.jsp" %>" />
            <listing:header_cell name="Reel Tag" column="<%= Reel.REEL_TAG_COLUMN %>" ascending="<%= "" + ascending %>" match="<%= column %>" url="<%= "received/checked_out.jsp" %>" />
            <listing:header_cell name="Cable Description" column="<%= Reel.CABLE_DESCRIPTION_COLUMN %>" ascending="<%= "" + ascending %>" match="<%= column %>" url="<%= "received/checked_out.jsp" %>" />
            <listing:header_cell width="75" name="Status" column="<%= Reel.STATUS_COLUMN %>" ascending="<%= "" + ascending %>" match="<%= column %>" url="<%= "received/checked_out.jsp" %>" />
            <listing:header_cell width="40" name="ID" column="<%= Reel.ID_COLUMN %>" ascending="<%= "" + ascending %>" match="<%= column %>" url="<%= "received/checked_out.jsp" %>" />
...
<admin:set_tabset url="received/_tabset_default.jsp" thispage="checked_out.jsp" />
<html:end />    