...
if(request.getParameter("pageNum") != null) {
    ...
    session.setAttribute("received/checked_out.jsp", pageNum);
} else {
    if(session.getAttribute("received/checked_out.jsp") != null) {
        pageNum = (Integer)session.getAttribute("received/checked_out.jsp");
    }
}
...
<% dbResources.close(); %>
<html:begin />
<admin:title heading="Reels" text="Checked Out" />
...
<% if(dosearch) { %>
    ...
    <admin:box_begin text="Reels" name="results" url="received/checked_out.jsp" pages="<%= Integer.toString(pages) %>" pageNum="<%= Integer.toString(pageNum) %>" />


    <listing:begin />
        <listing:header_begin />
            <listing:header_cell width="55" first="true" name="CRID #" column="<%= Reel.CR_ID_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/checked_out.jsp" />
