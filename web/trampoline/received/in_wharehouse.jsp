...
if(request.getParameter("pageNum") != null) {
    ...
    session.setAttribute("received/in_wharehouse.jsp", pageNum);
} else {
    if(session.getAttribute("received/in_wharehouse.jsp") != null) {
        pageNum = (Integer)session.getAttribute("received/in_wharehouse.jsp");
    }
}
...
<% dbResources.close(); %>
<html:begin />
<admin:title heading="Reels" text="In Warehouse" />
...
<% if(dosearch) { %>
    ...
    <admin:box_begin text="Reels" name="results" url="received/in_wharehouse.jsp" pages="<%= Integer.toString(pages) %>" pageNum="<%= Integer.toString(pageNum) %>" />


    <listing:begin />
        <listing:header_begin />
            <listing:header_cell width="55" first="true" name="CRID #" column="<%= Reel.CR_ID_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/in_wharehouse.jsp" />
            <listing:header_cell name="Reel Tag" column="<%= Reel.REEL_TAG_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/in_wharehouse.jsp" />
            <listing:header_cell name="Cable Description" column="<%= Reel.CABLE_DESCRIPTION_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/in_wharehouse.jsp" />
            <listing:header_cell width="75" name="Status" column="<%= Reel.STATUS_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="received/in_wharehouse.jsp" />
