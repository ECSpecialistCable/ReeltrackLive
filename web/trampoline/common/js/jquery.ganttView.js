	...
	var Chart = function(div, opts) {
        ...
        function addVtHeader(div, data, cellHeight) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                }).append(jQuery("<h5>").append(data[i].name)));
        ...
        function addGrid(div, data, dates, cellWidth, showWeekends) {
            ...
            for (var i = 0; i < data.length; i++) {
				...
				var gridSet = jQuery("<div>", { "class": "ganttview-grid-set" });
				var rowDivHeader = jQuery("<div>", { "class": "ganttview-grid-set-header" }); 
				
				//rowDivHeader.append( jQuery("<H3>").append(data[i].name));
				rowDivHeader.css("width", w + "px");
				//add a project header
				gridSet.append(rowDivHeader);
				
                for (var j = 0; j < data[i].series.length; j++) {
	
                   gridSet.append(rowDiv.clone());
                }
				gridDiv.append(gridSet);
            }
            div.append(gridDiv);
        }
        ...
        function addBlockContainers(div, data) {
            ...
            for (var i = 0; i < data.length; i++) {
				var blocksSetHeader = jQuery("<div>", { "class": "ganttview-blocks-set-header" });
				var blockSet = jQuery("<div>", { "class": "ganttview-blocks-set" });
				blockSet.append(blocksSetHeader);
                for (var j = 0; j < data[i].series.length; j++) {
	
                    blockSet.append(jQuery("<div>", { "class": "ganttview-block-container" }));
                }
				blocksDiv.append(blockSet);
            }
        ...
        function addBlocks(div, data, cellWidth, start) {
			...
			var sets = jQuery("div.ganttview-blocks div.ganttview-blocks-set", div);
            ...
            var rowIdx = 0;
            for (var i = 0; i < data.length; i++) {
				var containers = jQuery("div.ganttview-block-container", sets[setIdx])
                for (var j = 0; j < data[i].series.length; j++) {
					
					var row = jQuery("<div>", { "class": "ganttview-blocks-row" });
					...
					jQuery(containers[j]).append(row);
                }
				setIdx = setIdx + 1;
