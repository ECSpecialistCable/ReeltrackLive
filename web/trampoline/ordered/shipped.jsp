...
if(request.getParameter("pageNum") != null) {
    ...
    session.setAttribute("ordered/shipped.jsp", pageNum);
} else {
    if(session.getAttribute("ordered/shipped.jsp") != null) {
        pageNum = (Integer)session.getAttribute("ordered/shipped.jsp");
    }
}
...
<% dbResources.close(); %>
<html:begin />
<admin:title heading="Reels" text="Shipped" />
...
<% if(dosearch) { %>
    ...
    <admin:box_begin text="Reels" name="results" url="ordered/shipped.jsp" pages="<%= Integer.toString(pages) %>" pageNum="<%= Integer.toString(pageNum) %>" />
