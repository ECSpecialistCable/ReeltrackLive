...
if(request.getParameter("pageNum") != null) {
    ...
    session.setAttribute("ordered/refused.jsp", pageNum);
} else {
    if(session.getAttribute("ordered/refused.jsp") != null) {
        pageNum = (Integer)session.getAttribute("ordered/refused.jsp");
    }
}
...
if(session.getAttribute("ordered_refused")!=null) {
    content = (Reel)session.getAttribute("ordered_refused");
}
...
session.setAttribute("ordered_refused",content);
...
<% dbResources.close(); %>
<html:begin />
<admin:title heading="Reels" text="Refused" />

<admin:subtitle text="Search" />
    <admin:box_begin text="Search Reels"  name="Search" open="false"/>
    <form:begin_selfsubmit name="search" action="ordered/refused.jsp" />
...
<% if(dosearch) { %>
    ...
    <admin:box_begin text="Reels" name="results" url="ordered/refused.jsp" pages="<%= Integer.toString(pages) %>" pageNum="<%= Integer.toString(pageNum) %>" />

    <listing:begin />
        <listing:header_begin />
            <listing:header_cell width="55" first="true" name="CRID #" column="<%= Reel.CR_ID_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="ordered/refused.jsp"  />
            <listing:header_cell name="Reel Tag" column="<%= Reel.REEL_TAG_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="ordered/refused.jsp" />
            <listing:header_cell name="Cable Description" column="<%= Reel.CABLE_DESCRIPTION_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="ordered/refused.jsp"  />
            <listing:header_cell width="75" name="Status" column="<%= Reel.STATUS_COLUMN %>" ascending="<%= new Boolean(ascending).toString() %>" match="<%= column %>" url="ordered/refused.jsp" />
...
<admin:set_tabset url="ordered/_tabset_default.jsp" thispage="refused.jsp" />
<html:end />
